let question_arr = ['Яке значення коефіцієнту кореляції віповідає найкращій моделі?','Що таке гетероскедастичність?','Для виявлення мультиколлінеарності використовують:','У якому випадку проблеми мультиколлінеарності не існує:','Скільки фіктивних змінних треба ввести, якщо якісна зміна має k альтернативних значень?'];
let a1_arr = ["-0,9","Кореляція залишків між собою","Метод Голдфелда-Квандта","F_кр > F_розр","k-1"]
let a2_arr = ["0,8","Непостійна дисперсія","Критерій Кохрана-Оркатта","F_кр < F_розр","k"]
let a3_arr = ["0,2","Постійна дисперсія","Критерій Дарбіна-Уотсона","F_кр = F_розр","k+1"]
let a4_arr = ["-0,5","Взаємозв'язок між двома або більше пояснювальних змінних","Метод Феррара-Глобера","F_кр >= F_розр","k+2"]
let answer_arr = ["-0,9","Непостійна дисперсія","Метод Феррара-Глобера","F_кр > F_розр","k-1"]
let n_right_answer_arr = [1,2,4,1,1]
let right_answers = 0;
let n_answer = 4;
